...
* Front SD, USB device (wii), SDGecko (wii/gamecube), SD2SP2 and GCLoader (gamecube) support
* FAT16, FAT32 and exFAT storage support (no need to reformat large cards to FAT32 anymore)
...
* exFAT support for every device (Wii SD/USB, SD Gecko, SD2SP2, GCLoader).
  Cards larger than 32GB no longer need to be reformatted to FAT32.
- Switched from libfat to libdvm (Extrems' fork), which bundles FatFs and provides
  FAT12/16/32 + exFAT. See the COMPILING section, it is a new build requirement.
- Mounting now probes the partition table and picks the matching filesystem driver
  instead of assuming FAT, so a device whose first partition is not FAT (or holds a
  filesystem GCMM has no driver for) is handled properly.
* Fixed the device path not being cleared on unmount, which left a stale drive name
  behind after using the device selector.
...
About filesystems:
* FAT16, FAT32 and exFAT are supported. Only one partition of a device is used: the
  first one holding a filesystem GCMM can read.
* FAT32 is still the safest choice if you have a say in it, as it is what the rest of
  the GameCube/Wii homebrew scene expects, so a FAT32 card will work everywhere.
  Windows refuses to format volumes bigger than 32GB as FAT32, but that is a limit of
  the Windows format dialog, not of FAT32 itself (which goes up to 2TB). Tools like
  guiformat or Rufus will do it.
* exFAT is here so that a large card that came exFAT formatted just works without
  reformatting. GCMM only ever writes small files (a .gci is a few KB, a full raw card
  image is 16MB at most), so it gains nothing else from exFAT.
...
* Storage is mounted through libdvm, not libfat. Do not use fatMountSimple(): libdvm
  provides it for source compatibility, but it is hardcoded to the "vfat" driver and
  so it fails on an exFAT device. GCMM uses dvmProbeMountDiscIface() instead (see
  mountDevice() in main.c), which reads the partition table, identifies the filesystem
  of each partition and mounts it with the matching driver. Everything above that
  (fopen, opendir, mkdir...) is unchanged, it is all newlib devoptab.
* libdvm names the first partition of a device after the basename it is given ("sda",
  "wsd"...) and appends the partition number to the rest ("sda2", "sda3"). That is why
  fatpath is 8 bytes and not 4, and why deinitFAT() walks the partitions.
* exFAT is covered by Microsoft patents, which is why Extrems ships libogc2-libfat and
  libogc2-libdvm as separate packages. Keep it in mind if you distribute builds.
...
* libdvm (Extrems' fork): https://github.com/extremscorner/libdvm
	- This replaces libfat and is REQUIRED, GCMM no longer builds against libogc2-libfat.
	  It is what provides exFAT (it bundles ChaN's FatFs and lwext4).
	- It still installs as libfat.a, so the -lfat in the makefiles stays as it is.
...
The libogc2 pacman repository has to be added first. Import and locally sign the GPG
key (prefix with sudo everywhere except Windows/MSYS2):
...
Then append the repository to pacman.conf. It MUST be placed before [dkp-libs].

	[libogc2-devkitpro]
	Server = https://packages.libogc2.org/devkitpro/<os>/$arch
	Server = https://packages.extremscorner.org/devkitpro/<os>/$arch

Replace <os> with windows, linux or macos. pacman.conf lives in:
	Windows  - /etc/pacman.conf relative to the MSYS2 installation
	Linux    - /etc/pacman.conf (pacman) or /opt/devkitpro/pacman/etc/pacman.conf (dkp-pacman)
	macOS    - /opt/devkitpro/pacman/etc/pacman.conf

Full instructions: https://github.com/extremscorner/pacman-packages#readme

Installing the dependencies:

	(dkp-)pacman -S gamecube-tools-git libogc2 libogc2-libdvm-git ppc-freetype

libogc2-libdvm-git conflicts with libogc2-libfat, pacman will offer to replace it.
Prebuilt packages are published for Windows (MSYS2), Linux and macOS alike, so none
of them needs the package to be built locally, and WSL is not required on Windows.

Note: recent ppc-freetype no longer ships freetype-config, it ships a pkg-config file
instead. The makefiles still call freetype-config, so on an up to date toolchain they
have to use `powerpc-eabi-pkg-config freetype2 --cflags/--libs` instead (that binary
comes from libogc2-pkg-config and the libogc2 *_rules already put it in PATH).

Building:

	make         - builds both the Wii and the GameCube dol (light theme)
	make dark    - same, dark theme
	make wii     - Wii only
...
* Extrems for many libogc bugfixes, gcloader and sd2sp2 support in libogc, libfat updates and improvements, and for libdvm, which is what makes exFAT support possible
* fincs and devkitPro for libdvm, and ChaN for FatFs
